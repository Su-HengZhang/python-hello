...
if __name__ == "__main__":
    ...
    data = []
    for _ in range(12):
        data.extend([3.2] * 24)   # 高电平
        data.extend([0.4] * 24)   # 低电平
    ...
    df, segments, flip_count = analyze_signal_levels_pandas(
        data, threshold=1.5, min_run_length=5
    )
    ...
    # 显示段信息（精简版）
    print(segments[['segment_id', 'level_str', 'start', 'end', 
                   'length', 'mean_voltage']].round(3))
